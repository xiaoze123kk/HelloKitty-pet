...
v0.11.1 让 Windows 原生拖拽在松开鼠标后约 60–100ms 内进入落地反馈；按住不动仍保持悬挂，
不会因为缩短静默时间而提前落地。原来的 700ms 静默判断仅作为系统按键探测失败时的兜底。

v0.12.0 引入 Behavior Context 2.0：行为脑会读取最近的自主行为与动作、当前互动类型、互动连续度、
关系阶段、时间段和本次会话阶段。旧版行为历史会无损迁移；细分触摸目标只保留在当前会话，
不会记录点击坐标或新增个人信息。

v0.12.1 让已保存的行为历史真正参与选择：刚发生的同类行为会受到明确的重复惩罚，近期未出现
的行为获得轻量 novelty 加成。冷却、勿扰、设置和动画开关仍是硬门禁，开发环境可检查每项分数
由需求、处境、关系、性格、重复、novelty 和随机扰动中的哪一部分构成。

v0.12.2 增加独立 Behavior Director：行为脑只选择高层意图，导演再从带权目录中编排 1–3 个
...
v0.12.4 增加 Expression Director：相同高层意图会根据处境和近期动作解析成不同 motion，并统一指定
